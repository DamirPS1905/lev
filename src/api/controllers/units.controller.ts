...
import { Body, Delete, Get, HttpException, HttpStatus, Param, ParseIntPipe, Patch, Post } from '@nestjs/common';
...
export class UnitsController extends GenUnitsController {
  ...
  async validateCreate(actor: Actors, group: number, createDto: CreateUnitDto) {
  ...
  async validateUpdate(entity, actor: Actors, group: number, id: number, updateDto: UpdateUnitDto) {
    ...
    if (updateDto.title !== undefined && updateDto.title !== entity.title) {
      ...
      const existedMyT = await this.unitsService.findByGroupAndCompanyAndTitle(
        group,
        actor.company.id,
        updateDto.title,
      );
