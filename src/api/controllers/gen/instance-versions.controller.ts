...
export class GenInstanceVersionsController {
  ...
  async findAll(actor: Actors, offset: number, limit: number) {
    if (offset < 0)
      throw new HttpException('Wrong offset value', HttpStatus.BAD_REQUEST);
    if (limit < 0)
      throw new HttpException('Wrong limit value', HttpStatus.BAD_REQUEST);
    if (limit > 1000) limit = 1000;
    return await this.instanceVersionsService.listByCompany(
      actor.company.id,
      offset,
      limit,
    );
