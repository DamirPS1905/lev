...
export class GenProductPropertyValuesController {
	...
	async update(actor: Actors, catalog: number, product: bigint, property: number, order: number, updateDto: UpdateProductPropertyValueDto) {
		...
		return await this.productPropertyValuesService.transactional(async (em) => {
			...
			await this.validateUpdate(entity, actor, catalog, product, property, order, updateDto, em);
			if(entity!==null){
	...
	async validateUpdate(entity, actor: Actors, catalog: number, product: bigint, property: number, order: number, updateDto: UpdateProductPropertyValueDto, em: EntityManager) { }
	...
	async delete(actor: Actors, catalog: number, product: bigint, property: number, order: number) {
		...
		return await this.productPropertyValuesService.transactional(async (em) => {
			...
			await this.validateDelete(entity, actor, catalog, product, property, order, em);
			return await this.productPropertyValuesService.remove(entity, em);
		});
	}
	
	async validateDelete(entity, actor: Actors, catalog: number, product: bigint, property: number, order: number, em: EntityManager) { }
