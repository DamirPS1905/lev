...
export class RatesController extends GenRatesController {
	...
	async findOne(@AuthInfo() actor: Actors, @Param('from') from: string, @Param('to') to: string, @Param('source', ParseIntPipe) sourceId: number) {
		...
		if(fromCurrency.id!==source.baseCurrency.id){
			const toBase = await this.ratesService.findByFromAndToAndSource(fromCurrency.id, source.baseCurrency.id, source.id);
			if(toBase===null){
		...
		if(toCurrency.id!==source.baseCurrency.id){
			const fromBase = await this.ratesService.findByFromAndToAndSource(source.baseCurrency.id, toCurrency.id, source.id);
			if(fromBase===null){
				throw new HttpException('Unable detrminate rate using current source', HttpStatus.NOT_FOUND);
			}
